 ...
 * @version 2.5.8
 * @description Required Library for CozLex :>'s Plugins
 */
		...
		load () {
			...
			BDFDB.PluginUtils.openChangeLog = function (plugin) {
				...
				if (changeLogHTML) BDFDB.ModalUtils.open(plugin, {
					...
					footerChildren: (plugin == BDFDB || plugin == this || PluginStores.loaded[plugin.name] && PluginStores.loaded[plugin.name] == plugin && plugin.author == "CozLex :>") && BDFDB.ReactUtils.createElement("div", {
			...
			const requestLibraryHashes = tryAgain => {
				request("https://api.github.com/repos/Huzukawaiii/BetterDiscordAddons/contents/Library/_res/", {headers: {"user-agent": "node.js"}}, (e, r, b) => {
			...
			const requestLibraryData = tryAgain => {
				const parseCSS = css => {
					...
					else request.get(`https://raw.githubusercontent.com/Huzukawaiii/BetterDiscordAddons/Library/_res/${dataFileName}`, (e, r, b) => {
				...
				else request.get(`https://raw.githubusercontent.com/Huzukawaiii/BetterDiscordAddons/Library/_res/${cssFileName}`, (e, r, b) => {
					if ((e || !b || r.statusCode != 200) && tryAgain) return BDFDB.TimeUtils.timeout(_ => requestLibraryData(), 10000);
					if (!e && b && r.statusCode == 200) {
						if (backupObj.backup && backupObj.backup.replace(/\s/g, "") == b.replace(/\s/g, "")) {
				...
				Internal.getPluginURL = function (plugin) {
					...
					if (BDFDB.ObjectUtils.is(plugin)) {
						if (InternalData.PluginUrlMap && InternalData.PluginUrlMap[plugin.name]) return InternalData.PluginUrlMap[plugin.name];
							...
							return `https://raw.githubusercontent.com/Huzukawaiii/BetterDiscordAddons/Plugins/${name}/${name}.plugin.js`;
						}
					}
					else return "";
				};
